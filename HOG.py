...
def visualize_face_detection(I_target,bounding_boxes,box_size):
    ...
    for ii in range(bounding_boxes.shape[0]):
        ...
        fimg = cv2.rectangle(fimg, (int(x1),int(y1)), (int(x2),int(y2)), (255, 0, 0), 2)
        # cv2.putText(fimg, "%.2f"%bounding_boxes[ii,2], (int(x1)+1, int(y1)+2), cv2.FONT_HERSHEY_SIMPLEX , 0.5, (0, 255, 0), 2)

    plt.figure(3)
    plt.imshow(fimg, vmin=0, vmax=1)
    plt.savefig('HOG_face_detection.png')
...
if __name__=='__main__':
    ...
    I_target= cv2.imread('0001.jpg', 0)
    #MxN image

    I_template = cv2.imread('2.png', 0)
    #mxn  face template

    bounding_boxes=face_recognition(I_target, I_template)

    I_target_c= cv2.imread('0001.jpg')
    ...
    img=cv2.imread("HOG_face_detection.png")
